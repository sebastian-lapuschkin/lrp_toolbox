...
We offer a \emph{Singularity} image which enables a convenient way to calculate \emph{LRP} relevance scores for pre-trained imagenet models using \emph{caffe}.
% example the installation process and workflow for the larger imagenet models, facilitates getting started
The image can be used as an example for the installation process and workflow and facilitates getting started with the \emph{caffe} toolbox.
...
% reproducable
For more material on \emph{Singularity} , we refer to the \emph{sylabs} website (\url{https://sylabs.io/singularity/}).
...
%run the demo
The demo calculates relevance scores for several images and stores them as heatmaps. The demo is started by calling
\begin{Verbatim}[frame = single]
cd <toolbox_location>/singularity
bash lrp_demo.sh 
\end{Verbatim}
and results in are stored in \verb|lrp_demo/lrp_output/someimages/|.
...
For more information about the \emph{LRP} parameter options below in Section~\nameref{sec:caffe_specifics}.
