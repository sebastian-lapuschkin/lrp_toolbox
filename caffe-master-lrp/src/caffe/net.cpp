...
void Net<Dtype>::Init(const NetParameter& in_param) {
	...
	for (int input_id = 0; input_id < param.input_size(); ++input_id) {
		...
		AppendTop(param, layer_id, input_id, &available_blobs,
				&blob_name_to_idx);
	}
	DLOG_IF(INFO, Caffe::root_solver()) << "Memory required for data: "
	...
	for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
		...
		if (share_from_root) {
			LOG(INFO) << "Sharing layer " << layer_param.name()
					<< " from root net";
		...
		for (int top_id = 0; top_id < num_top; ++top_id) {
			AppendTop(param, layer_id, top_id, &available_blobs,
					&blob_name_to_idx);
		}
		...
		if (share_from_root) {
			...
			const vector<Blob<Dtype>*>& base_top =
					root_net_->top_vecs_[layer_id];
			const vector<Blob<Dtype>*>& this_top = this->top_vecs_[layer_id];
			for (int top_id = 0; top_id < base_top.size(); ++top_id) {
				this_top[top_id]->ReshapeLike(*base_top[top_id]);
				LOG(INFO) << "Created top blob " << top_id << " (shape: "
						<< this_top[top_id]->shape_string()
						<< ") for shared layer " << layer_param.name();
			}
		} else {
		...
		for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
			...
			blob_loss_weights_[top_id_vecs_[layer_id][top_id]] = layer->loss(
					top_id);
			if (Caffe::root_solver()) {
				LOG(INFO) << "Top shape: "
		...
		if (need_backward) {
			for (int top_id = 0; top_id < top_id_vecs_[layer_id].size();
					++top_id) {
	...
	for (int layer_id = layers_.size() - 1; layer_id >= 0; --layer_id) {
		...
		for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
			...
			if (layers_[layer_id]->loss(top_id)
					|| (blobs_under_loss.find(blob_name)
							!= blobs_under_loss.end())) {
		...
		for (int layer_id = 0; layer_id < layers_.size(); ++layer_id) {
			...
			for (int bottom_id = 0;
				...
				bottom_need_backward_[layer_id][bottom_id] =
						bottom_need_backward_[layer_id][bottom_id]
								|| layers_[layer_id]->AllowForceBackward(
										bottom_id);
	...
	if (Caffe::root_solver()) {
		...
		LOG(INFO) << "Memory required for data: "
				<< memory_used_ * sizeof(Dtype);
	}
}
...
void Net<Dtype>::FilterNet(const NetParameter& param,
	...
	for (int i = 0; i < param.layer_size(); ++i) {
		...
		CHECK(
				layer_param.include_size() == 0
						|| layer_param.exclude_size() == 0)
		...
		for (int j = 0; !layer_included && j < layer_param.include_size();
				++j) {
...
bool Net<Dtype>::StateMeetsRule(const NetState& state, const NetStateRule& rule,
	...
	for (int i = 0; i < rule.stage_size(); ++i) {
		...
		if (!has_stage) {
			if (Caffe::root_solver()) {
				LOG(INFO) << "The NetState did not contain stage '"
						<< rule.stage(i) << "' specified by a rule in layer "
						<< layer_name;
			}
			return false;
		}
	...
	for (int i = 0; i < rule.not_stage_size(); ++i) {
		...
		if (has_stage) {
			if (Caffe::root_solver()) {
				LOG(INFO) << "The NetState contained a not_stage '"
						<< rule.not_stage(i)
						<< "' specified by a rule in layer " << layer_name;
			}
			return false;
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,
	...
	ParamSpec default_param_spec;
	const ParamSpec* param_spec =
			(layer_param.param_size() > param_id) ?
					&layer_param.param(param_id) : &default_param_spec;
	if (!param_size || !param_name.size()
			|| (param_name.size()
					&& param_names_index_.find(param_name)
							== param_names_index_.end())) {
		...
		LOG_IF(INFO, Caffe::root_solver()) << "Sharing parameters '"
				<< param_name << "' owned by " << "layer '"
				<< layer_names_[owner_layer_id] << "', param " << "index "
				<< owner_param_id;
		...
		if (param_size > param_id
			...
			CHECK_EQ(this_blob->count(), owner_blob->count())
					...
					<< "' owned by layer '" << layer_names_[owner_layer_id]
					<< "' with layer '" << layer_names_[layer_id]
					<< "'; count mismatch.  Owner layer param " << "shape is "
					<< owner_blob->shape_string() << "; sharing layer "
			...
			CHECK(this_blob->shape() == owner_blob->shape())
					...
					<< "' owned by layer '" << layer_names_[owner_layer_id]
					<< "' with layer '" << layer_names_[layer_id]
					<< "'; shape mismatch.  Owner layer param " << "shape is "
					<< owner_blob->shape_string() << "; sharing layer "
...
template<typename Dtype>
void Net<Dtype>::Backward_Relevance_multi(const std::vector< std::vector<int> > & classinds,
	...
	for (int i = lastlayerindex; i >= firstlayerindex; --i) {
		...
		//if (layer_need_backward_[i]) {
			...
			layers_[i]->Backward_Relevance_cpu(top_vecs_[i], bottom_need_backward_[i], bottom_vecs_[i],
				i, ro, classinds[0], thenightstartshere);
...
void Net<Dtype>::UpdateDebugInfo(const int param_id) {
	...
	const string& layer_name =
			layer_names_[param_layer_indices_[param_id].first];
	...
	if (param_owner < 0) {
		...
		if (Caffe::root_solver()) {
			LOG(INFO) << "    [Update] Layer " << layer_name << ", param blob "
					<< param_display_name << " (owned by layer "
					<< owner_layer_name << ", " << "param "
...
void Net<Dtype>::CopyTrainedLayersFrom(const string trained_filename) {
	if (trained_filename.size() >= 3
			&& trained_filename.compare(trained_filename.size() - 3, 3, ".h5")
					== 0) {
...
void Net<Dtype>::CopyTrainedLayersFromHDF5(const string trained_filename) {
	...
	for (int i = 0; i < num_layers; ++i) {
		...
		hid_t layer_hid = H5Gopen2(data_hid, source_layer_name.c_str(),
				H5P_DEFAULT);
		CHECK_GE(layer_hid, 0) << "Error reading weights from "
				<< trained_filename;
		...
		for (int j = 0; j < target_blobs.size(); ++j) {
			...
			hdf5_load_nd_dataset(layer_hid, dataset_name.c_str(), 0,
					kMaxBlobAxes, target_blobs[j].get());
		}
		H5Gclose(layer_hid);
	}
	H5Gclose(data_hid);
...
void Net<Dtype>::ToHDF5(const string& filename, bool write_diff) const {
	hid_t file_hid = H5Fcreate(filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
			H5P_DEFAULT);
	CHECK_GE(file_hid, 0) << "Couldn't open " << filename
			<< " to save weights.";
	...
	for (int layer_id = 0; layer_id < layers_.size(); ++layer_id) {
		...
		if (write_diff) {
			layer_diff_hid = H5Gcreate2(diff_hid, layer_name.c_str(),
					H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
			CHECK_GE(layer_diff_hid, 0) << "Error saving weights to "
					<< filename << ".";
		}
		...
		for (int param_id = 0; param_id < num_params; ++param_id) {
			...
			if (param_owners_[net_param_id] == -1) {
				...
				hdf5_save_nd_dataset < Dtype
						> (layer_data_hid, dataset_name.str(), *params_[net_param_id]);
			}
			if (write_diff) {
				// Write diffs regardless of weight-sharing
				hdf5_save_nd_dataset < Dtype
						> (layer_diff_hid, dataset_name.str(), *params_[net_param_id], true);
			}
		}
