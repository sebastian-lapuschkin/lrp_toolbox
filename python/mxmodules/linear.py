...
class Linear(Module):
    ...
    def _simple_lrp(self,R):
        ...
        bs = self.X.shape[0] # batchsize
        Z  = self.W.reshape((1,self.m,self.n)) * self.X.reshape((bs, self.m, 1)) # localized preactivations
        Zs = nd.sum(Z, axis=1).reshape((bs, 1, self.n)) + self.B.reshape((1,1,self.n))
        return nd.sum((Z / Zs) * R.reshape((bs, 1, self.n)), axis=2)

    # numpy functions that are not yet translated to mxnet:
    # ####################################################
    #
